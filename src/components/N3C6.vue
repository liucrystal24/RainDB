  ...
  <div class="totalContainer">
    <div class="searchcontainer">
      <el-row class="searchtitle">
        ...
        <div class="datades">数据区间：2014.8.21~2018.9.7</div>
      </el-row>
...
export default {
  ...
  data() {
    return {
      dateselect: {
        startdate: "",
        starttime: "",
        startdefaultdate: new Date(2014, 7, 21),
        enddate: "",
        endtime: "",
        enddefaultdate: new Date(2014, 7, 30),
        stationid: ""
      },
      form: {
        stationnum: "",
  ...
  methods: {
    ...
    zdTable() {
      ...
      if (this.zdTitle.indexOf("站点编码") == -1) {
        this.zdTitle.unshift("站点编码", "观测时间 ObservTime");
      }
      ...
      this.axios
        ...
        .then(
          res => {
            ...
            this.loading = false;
            this.tableData = res.data.info.rainsearch;
          },
          res => {
            console.log("err");
            console.log(res);
    ...
    export2Excel() {
      require.ensure([], () => {
        ...
        if (tableLength != 0) {
          export_json_to_excel(tHeader, data, "海南雨量数据");
        } else {
      ...
      this.axios.get(url).then(
        res => {
          ...
          stationarr.forEach((val, index) => {
            ...
            let station = {};
            if (val["站点编码"] != null) {
              station.value = val["站点编码"];
              station.label = val["站点编码"];
    ...
    findTitle() {
      ...
      this.axios.get(url).then(
        res => {
          ...
          let titleObj = res.data.info.rainTitle[0];
          Object.keys(titleObj).forEach(key => {
            if (key != "站点编码" && key != "观测时间 ObservTime") {
              this.rainoptions.push({
                label: key,
                value: key
              });
            }
    ...
    selectAll(val) {
      let allValues = [];
      //保留所有值
      for (let item of this.rainoptions) {
